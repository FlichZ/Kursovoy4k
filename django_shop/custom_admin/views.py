...
def dashboard(request):
    ...
    registration_data, registration_labels = registration_activity_graph(
        registration_range, reg_start, reg_end
    )
    ...
    if request.method == "POST" and request.FILES.get("backup_file"):
        ...
        try:
            fs = FileSystemStorage(
                location="temp_backups"
            )  # Temporary folder for backups
            ...
            if backup_format == ".json":
                ...
                messages.error(
                    request,
                    "Unsupported file format. Please upload .json or .sql files.",
                )
...
def registration_activity_graph(time_range, start_date=None, end_date=None):
    ...
    if start_date and end_date:
        labels = [
            (start_date + datetime.timedelta(days=i)).strftime("%d %b")
            for i in range((end_date - start_date).days + 1)
        ]
        registration_data = [
            User.objects.filter(
                date_joined__date=start_date + datetime.timedelta(days=i)
            ).count()
            for i in range((end_date - start_date).days + 1)
        ]
        ...
        end_date = today
        labels = [
            (start_date + datetime.timedelta(days=i)).strftime("%A") for i in range(7)
        ]
        registration_data = [
            User.objects.filter(
                date_joined__date=start_date + datetime.timedelta(days=i)
        ...
        labels = [
            datetime.date(today.year, month, 1).strftime("%B") for month in range(1, 13)
        ]
        registration_data = [
            User.objects.filter(
                date_joined__year=today.year, date_joined__month=month
            ).count()
    ...
    if start_date and end_date:
        ...
        order_data = [
            Order.objects.filter(
                created__date=start_date + datetime.timedelta(days=i)
            ).count()
            for i in range((end_date - start_date).days + 1)
        ]
    elif time_range == "week":
        ...
        order_data = [
            Order.objects.filter(created__year=today.year, created__month=month).count()
            for month in range(1, 13)
        ]
    else:  # По умолчанию месяц
        start_date = today.replace(day=1)
...
def user_list(request):
    ...
    return render(
        request,
        "custom_admin/CRUD/user_list.html",
...
def product_list(request):
    ...
    return render(
        request,
        "custom_admin/crud/product_list.html",
        {
...
def product_create(request):
    ...
    if request.method == "POST":
        ...
        if not title or not slug or not price or not description or not category:
            ...
            return render(
                request,
                "custom_admin/crud/product_form.html",
    ...
    if request.method == "POST":
        ...
        if not title or not slug or not price or not description or not category:
            ...
            return render(
                request,
                "custom_admin/crud/product_form.html",
                {"categories": categories, "tags": tags, "product": product},
            )
...
def filter_list(request):
    ...
    return render(
        request, "custom_admin/crud/filter_list.html", {"filters": paginated_filters}
    )
...
def filter_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if not name or not slug:
            messages.error(request, "Имя и слаг фильтра обязательны для заполнения.")
            return render(
                request, "custom_admin/crud/filter_form.html", {"filter": filter_obj}
            )
...
def review_list(request):
    ...
    return render(
        request, "custom_admin/crud/review_list.html", {"reviews": paginated_reviews}
    )
...
def review_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if not content or not product:
            messages.error(request, "Все поля должны быть заполнены.")
            return render(
                request, "custom_admin/crud/review_form.html", {"review": review}
            )
    ...
    return render(
        request,
        "custom_admin/crud/review_form.html",
...
def category_list(request):
    ...
    return render(
        request,
        "custom_admin/crud/category_list.html",
...
def category_edit(request, id):
    ...
    if request.method == "POST":
        ...
        return redirect("custom_admin:category_list")
    return render(
        request, "custom_admin/crud/category_form.html", {"category": category}
    )
...
def order_list(request):
    ...
    if query:
        orders = (
            orders.filter(first_name__icontains=query)
            | orders.filter(last_name__icontains=query)
            | orders.filter(email__icontains=query)
            | orders.filter(city__icontains=query)
        )
    ...
    return render(
        request,
        "custom_admin/crud/order_list.html",
        {
...
def orderitem_list(request):
    ...
    return render(
        request,
        "custom_admin/crud/orderitem_list.html",
...
def orderitem_edit(request, id):
    ...
    return render(
        request, "custom_admin/crud/orderitem_form.html", {"order_item": order_item}
    )
