...
class RegisterUserFormTest(TestCase):
    ...
    def test_form_invalid_email(self):
        ...
        self.assertIn(
            "email", form.errors
        )  # Если ключ 'email' есть в form.errors, значит, при валидации формы возникла ошибка в поле 'email'.
...
class LoginUserFormTest(TestCase):
    def setUp(self):
        ...
        self.invalid_data = {"username": "testuser", "password": "wrongpass"}
    ...
    def test_login_view_valid(self):
        ...
        response = self.client.post(
            reverse("shop:login"), data=self.valid_data, follow=True
        )  # follow=True означает, что приложение должно следовать любым редиректам, которые могут быть выполнены в процессе выполнения запроса.
        user = authenticate(
            username=self.valid_data["username"], password=self.valid_data["password"]
        )
    ...
    def test_login_view_invalid(self):
        ...
        response = self.client.post(
            reverse("shop:login"), data=self.invalid_data, follow=True
        )
        user = authenticate(
            username=self.invalid_data["username"],
            password=self.invalid_data["password"],
        )
...
class ReviewFormTest(TestCase):
    ...
    def test_valid_review_form_submission(self):
        form_data = {"text": "Test review text", "rating": 4}
