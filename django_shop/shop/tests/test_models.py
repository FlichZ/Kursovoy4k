...
class CategoryModelTest(TestCase):
    ...
    def test_category_ordering(self):
        ...
        self.assertEqual(
            list(categories), [categories[0], categories[1], self.category]
        )
...
class ProductModelTest(TestCase):
    ...
    def test_get_absolute_url(self):
        url = reverse(
            "shop:product_detail", args=[self.category.slug, self.product.slug]
        )
...
class ReviewModelTest(TestCase):
    ...
    def test_review_creation(self):
        review = Review.objects.create(
            product=self.product, author="Test Author", rating=4, text="Test Text"
        )
    ...
    def test_review_invalid_rating(self):
        with self.assertRaises(ValidationError) as context:
            Review.objects.create(
                product=self.product, author="Test Author", rating=6, text="Test Text"
            )

    def test_review_valid_rating(self):
        review = Review.objects.create(
            product=self.product, author="Test Author", rating=1, text="Test Text"
        )
        self.assertEqual(review.rating, 1)

    def test_reviews_ordered_by_creation_date(self):
        Review.objects.create(
            product=self.product, author="Test Author 1", rating=5, text="Test Text 1"
        )
        Review.objects.create(
            product=self.product, author="Test Author 2", rating=3, text="Test Text 2"
        )
        Review.objects.create(
            product=self.product, author="Test Author 3", rating=4, text="Test Text 3"
        )
