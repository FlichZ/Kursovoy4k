...
class TestShopHome(TestCase):
    ...
    def test_get_queryset(self):
        ...
        self.assertQuerysetEqual(
            response.context["products"], [self.product], transform=lambda x: x
        )  # через lambda идет преобразование в  [<Product: Product>]
...
class ProductDetailViewTestCase(TestCase):
    ...
    def test_detail_view_context_cart_product_form(self):
        ...
        self.assertIsInstance(
            response.context["cart_product_form"], CartAddProductForm
        )  # проверка типа объекта на соответствие заданному классу
    ...
    def test_detail_view_post_valid_form(self):
        data = {"rating": 5, "text": "Test review text"}
...
class LogoutUserViewTest(TestCase):
    ...
    def test_logout_user_view_logs_out_user(self):
        self.client.post(
            reverse("shop:login"), {"username": "testuser", "password": "testpass"}
        )
