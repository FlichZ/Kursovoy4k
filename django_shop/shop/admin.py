...
class ProductAdmin(admin.ModelAdmin):
    ...
    prepopulated_fields = {
        "slug": ("title",)
    }  # cоздание слага на основе атрибута title
    search_fields = ["title"]
    ...
    def get_html_photo(
        self, object
    ):  # object тут ссылается на запись из таблицы (ЭК модели Product)
        if object.image:
            return mark_safe(
                f"<img src='{object.image.url}' width=65>"
            )  # ф-ия mark_safe позволяет не экранировать то, что мы в нее передаем
